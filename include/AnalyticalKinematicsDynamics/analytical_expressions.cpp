...
// left elbow
MatrixXd AnalyticalExpressions::p_left_elbow(VectorXd q)
{
  MatrixXd output(3,1);
  //input
  ...
  // set zero the matrix
  output.setZero();
  p_elbow_joint_left_src(output.data(), q.data());
  return output;
}

MatrixXd AnalyticalExpressions::Jp_left_elbow(VectorXd q)
{
  ...
  // set zero the matrix
  output.setZero();
  Jp_elbow_joint_left_src(output.data(), q.data());
  return output;
}

MatrixXd AnalyticalExpressions::p_right_toe_front(const VectorXd& q)
{
  MatrixXd output(3,1);
  //input
  assert_size_matrix(q, 28, 1);
  // set zero the matrix
  output.setZero();
  p_RightToeBottomFront_src(output.data(), q.data());
  return output;
}

MatrixXd AnalyticalExpressions::p_right_toe_back(const VectorXd& q)
{
  MatrixXd output(3,1);
  //input
  assert_size_matrix(q, 28, 1);
  ...
  // set zero the matrix
  output.setZero();
  p_RightToeBottomBack_src(output.data(), q.data());
  return output;
}

MatrixXd AnalyticalExpressions::Jp_right_toe_front(const VectorXd& q) {
  MatrixXd output(3, 28);
  assert_size_matrix(q, 28, 1);
  //assert_size_matrix(output, 3, 28);
...
MatrixXd AnalyticalExpressions::Jp_right_toe_back(const VectorXd& q) {
  MatrixXd output(3, 28);
  assert_size_matrix(q, 28, 1);
  //assert_size_matrix(output, 3, 28);
...
MatrixXd AnalyticalExpressions::p_left_toe_front(const VectorXd& q)
{
  MatrixXd output(3,1);
  //input
  assert_size_matrix(q, 28, 1);
  ...
  // set zero the matrix
  output.setZero();
  p_LeftToeBottomFront_src(output.data(), q.data());
  return output;
}

MatrixXd AnalyticalExpressions::p_left_toe_back(const VectorXd& q)
{
  MatrixXd output(3,1);
  //input
  assert_size_matrix(q, 28, 1);
  ...
  // set zero the matrix
  output.setZero();
  p_LeftToeBottomBack_src(output.data(), q.data());
  return output;
}

MatrixXd AnalyticalExpressions::Jp_left_toe_front(const VectorXd& q) {
  MatrixXd output(3, 28);
  assert_size_matrix(q, 28, 1);
  //assert_size_matrix(output, 3, 28);
...
MatrixXd AnalyticalExpressions::Jp_left_toe_back(const VectorXd& q) {
  MatrixXd output(3, 28);
  assert_size_matrix(q, 28, 1);
  //assert_size_matrix(output, 3, 28);
...
MatrixXd AnalyticalExpressions::dJp_left_toe_front(VectorXd q, VectorXd dq) {
  MatrixXd output(3, 28);
  assert_size_matrix(q, 28, 1);
  assert_size_matrix(output, 3, 28);

  dJp_LeftToeBottomFront_src(output.data(), q.data(),dq.data());
  return output;
}

MatrixXd AnalyticalExpressions::dJp_left_toe_back(VectorXd q, VectorXd dq) {
  MatrixXd output(3, 28);
  assert_size_matrix(q, 28, 1);
  assert_size_matrix(output, 3, 28);

  dJp_LeftToeBottomBack_src(output.data(), q.data(), dq.data());
  return output;
}

MatrixXd AnalyticalExpressions::dJp_right_toe_front(VectorXd q, VectorXd dq) {
  MatrixXd output(3, 28);
  assert_size_matrix(q, 28, 1);
  assert_size_matrix(output, 3, 28);

  dJp_RightToeBottomFront_src(output.data(), q.data(),dq.data());
  return output;
}

MatrixXd AnalyticalExpressions::dJp_right_toe_back(VectorXd q, VectorXd dq) {
  MatrixXd output(3, 28);
  assert_size_matrix(q, 28, 1);
  assert_size_matrix(output, 3, 28);

  dJp_RightToeBottomBack_src(output.data(), q.data(), dq.data());
  return output;
}
...
MatrixXd AnalyticalExpressions::p_left_toe_roll(VectorXd q)
{
  MatrixXd output(3,1);
  //input
  assert_size_matrix(q, 28, 1);
...
MatrixXd AnalyticalExpressions::p_right_toe_roll(VectorXd q)
{
  MatrixXd output(3,1);
  //input
  assert_size_matrix(q, 28, 1);
...
MatrixXd AnalyticalExpressions::Jp_toe_roll_joint_left(VectorXd q) {
  MatrixXd output(3, 28);
  assert_size_matrix(q, 28, 1);
  assert_size_matrix(output, 3, 28);

  Jp_toe_roll_joint_left_src(output.data(), q.data());
  return output;
}
...
MatrixXd AnalyticalExpressions::InertiaMatrix(const VectorXd& q) {
...
MatrixXd AnalyticalExpressions::GravityVector(const VectorXd& q) {
  MatrixXd output(20,1);
  assert_size_matrix(q, 20, 1);
  //assert_size_matrix(output, 20, 1);
